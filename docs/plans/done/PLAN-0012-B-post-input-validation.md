...
- 2026-07-19: 코드리뷰(`/code-review`) 반영. **M1(수용+문서화)**: 길이 백스톱이 `PostMapper`→`Post.reconstitute`→`new PostContent` 를 통해 *읽기(조회) 경로에서도* 실행됨을 확인. DB 컬럼(title `VARCHAR(255)`, body `TEXT`)이 VO 한도(200/10000)보다 큰 값을 담을 수 있어, 경계를 우회해 저장된 초과 행이나 *한도 인하 후* 기존 행은 조회 시 `InvalidPostContentException` 이 된다(읽기 가용성이 현재 한도에 커플링). **결정: 수용** — 현 데이터는 모두 경계(`@Size`)를 거쳤고 reconstitute 가 blank/null 을 이미 재검증하던 선재 동작의 확장이며 한도 인하는 별도 의제이므로, `PostContent` 상수 javadoc 에 커플링을 명시하고 그대로 둔다(대안: 관대한 복원 경로 / DB 컬럼 정합 — 한도 인하 시 재검토). **L1**(Create/Update DTO 메시지 문구 중복)은 DTO 2개 수준이라 수용(숫자는 상수 파생이라 안전). 코드 변경은 문서 주석뿐 — `check` 재실행 불필요(로직 무변).
