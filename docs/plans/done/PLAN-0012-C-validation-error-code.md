...
- 2026-08-02: **`REQUIRED` 를 `MISSING`/`BLANK` 로 분리하는 안 검토 → 미채택.** 기술적으로는 가능하다(`getInvalidValue()` 로 null 판별). 그럼에도 두지 않은 이유: ① **행동 차이가 없다** — §5.2 확장 규약("클라이언트가 다르게 *행동*해야 할 때만 새 코드")의 자체 적용. 두 경우 모두 클라이언트 동작은 "실질적인 값을 넣어라"로 동일하고 문구 차이는 `reason` 의 일. ② **클라이언트가 이미 아는 정보** — 폼 상태를 든 쪽이 클라이언트이므로 "보낸 값이 null 이었다"는 회신은 새 정보가 아니다. `code` 는 *서버가 무엇에 반대했는지*를 전하는 자리(`TOO_LONG`/`TOO_SHORT` 는 서버만 아는 한도 판정이라 가치가 있다). ③ **이름 함정** — Jackson 이 `{"k":null}` 과 키 부재를 모두 null 로 매핑해 `MISSING` 은 지킬 수 없는 약속(→ 유일하게 시간을 견디는 논거라 ADR §5.2 에 한 줄로 남김). ④ **검증 설정 누출** — Post 는 title `@NotBlank` / body `@NotNull`(빈 본문 허용)이라, 분리하면 `title=""`→`BLANK` 인데 `body=""`→위반 아님인 비대칭이 어휘로 드러난다. **재검토 트리거**: ⓐ 실제 화면이 두 경우에 다르게 동작해야 함이 확인되거나, ⓑ 공백 입력 비율 관측이 필요해질 때. 단 ⓑ 는 로그 페이로드가 응답과 같은 `ValidationError` 를 쓰므로 *로그에서만* 구분하는 우회가 불가능하다 — 관측 목적이어도 API 계약이 함께 움직인다(거부값 자체를 로그에 싣는 우회는 `password` 평문 노출이라 금지).
- 2026-08-02: **ADR 보강 분량 판단** — 위 ①②④ 는 §5.2 에 *이미 있는* 확장 규약의 적용 결과이고 ④ 는 부차적 관찰이라, ADR 에는 시간을 견디는 ③(이름 함정)만 남기고 나머지는 본 Execution Notes 로 보냈다. 규칙을 써놓고 첫 적용 사례마다 규칙을 재유도하면 사례가 늘 때마다 문서가 같은 비율로 부풀고 규칙 자체가 묻힌다(plan-lifecycle.md §Execution Notes 의 "ADR=지속되는 결정 / Plan=회고 자료" 분업). **별도 관찰**: ADR-0005 는 320행·amend 4회이고 §5 계열만 60행에 근접 — §5 계열이 한 번 더 커지면 "입력 검증 계약"을 별도 ADR 로 분리하는 것을 검토(0005 는 예외→응답 매핑에 집중). 본 Plan 에서는 조치하지 않음.
