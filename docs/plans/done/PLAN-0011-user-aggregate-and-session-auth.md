...
**Bean Validation** — `@Valid` 흐름과 정합. 현재 `@NotBlank` + `@Size`(길이)만 부착. **email 형식 / nickname 허용문자 `@Pattern` 은 미구현** — 그 형식 검증은 도메인 VO(`Email`/`Nickname`)가 책임지며, invalid 형식은 VO 까지 흘러 `INVALID_USER_CONTENT` 가 된다. 경계(@Pattern)·VO 의 *입력 검증 에러 계약 통일* 은 ADR-0005 "향후 ADR 후보"의 Known-Limitation 으로 보류(이번 범위 아님).
...
- 2026-06-08: 머지 직전 Codex 리뷰 round-2 (별도 커밋 0951c6f/748ed34/7c48854 로 main 반영). null/누락 password 로그인이 BCrypt(null) → 500 + enumeration oracle 이 되던 경로를 hasher 전 `AuthenticationFailedException`(401)로 차단. 로그인 body 파싱 실패를 `MalformedAuthenticationRequestException` → 400 `MALFORMED_REQUEST` 로 분기(인증 실패와 구분). 중복 가입 예외 context 에서 email/nickname 원문(PII) 제거 → `field` 명만(ADR-0005 §3). CSRF 전송 헤더명 ADR-0011 §6 `X-XSRF-TOKEN`→`X-CSRF-TOKEN` 정정. register `@Pattern` 미구현 명시. 무관한 로컬 권한(`settings.local.json`) 제거.
- 2026-06-09: **B5 (prod Secure 쿠키 테스트) 머지·archival 후 follow-up 추가** — 최초 리뷰 체크리스트의 B5 가 PLAN 본문/Files 에 반영되지 않아 누락됐던 항목. `SecureCookieIT` 가 `server.servlet.session.cookie.secure=true` 주입 시 CSRF(`XSRF-TOKEN`) 쿠키에 `Secure` 속성이 실제 적용되는지 검증 — 배포에서 Secure 누락 회귀 차단 (ADR-0011 §7 *기본값 의존 금지*).
