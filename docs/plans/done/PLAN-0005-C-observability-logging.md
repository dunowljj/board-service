...
### Phase 1 — TraceId 인프라 (필터 + MDC + 응답 헤더 + Query 위생)
...
- `adapter/in/web/observability/TraceIdFilter` 신설 (`OncePerRequestFilter` 상속).
  ...
  - MDC에 `traceId`, `method`, `path`, `query` 네 키 주입. `query`는 아래 위생 처리 결과 문자열.
  - 응답 헤더 `X-Trace-Id`로 동일 ID 노출 (ADR-0005 §4).
  - finally 블록에서 **본 필터가 put한 네 키만** `MDC.remove(...)` 호출. `MDC.clear()` **금지** — 다른 필터(Security/Tracing 등)가 같은 MDC를 쓸 때 그 키까지 함께 지워짐.
- **Query string 위생 처리 (allowlist)** — ADR-0005 §7 클라이언트 주입 입력 위생 규칙 구현.
  - **L1 (키 위생)**: 키가 정규식 `^[a-zA-Z][a-zA-Z0-9_-]{0,31}$`를 통과하지 못하면 `[invalid]` 카운트로만 흡수 (PII 형태 키 자동 차단).
  - **L2 (default-deny 익명화)**: 정규식은 통과했지만 allowlist에 없는 키는 `[N keys redacted]`로 카디널리티만 보존. 키 이름 자체의 PII 누출 방지.
  - **L3 (프로세스 가드)**: 신규 `@RequestParam` 추가 시 allowlist 동기화 체크 + ArchUnit 룰은 **본 Plan 범위 외**, 후속 test-strategy Plan에서 수행.
  - Allowlist 외부화: `observability.query.value-allowlist`를 `@ConfigurationProperties`로 바인딩(`QueryLogProperties`). `application.yml` default = `[page, size, sort]` (Spring Data 표준 페이지네이션 트라이플렛, 값이 PII가 될 수 없는 메타 파라미터).
  - 결과 형식 예: `?page=2&size=10&token=abc&ssn=123` → MDC `query="page=2,size=10,[2 keys redacted]"`.
- 필터 등록: **`FilterRegistrationBean`으로 명시 등록** (`@Component @Order` 미사용 — Spring Boot 자동 등록 경로에서 `@Order` 인식이 보장되지 않음).
  - `private static final int FILTER_ORDER = -101;` 상수 + javadoc로 의도 명시 ("`SecurityProperties.DEFAULT_FILTER_ORDER`(-100)보다 1 작아 추후 Spring Security 도입 시 그 앞에 위치").
  - Security starter 미도입 상태에서 `SecurityProperties` import를 미리 끌어오지 않도록 매직넘버를 자체 상수화 (ADR-0004 골격 우선 정신). Security ADR 도입 시 `SecurityProperties.DEFAULT_FILTER_ORDER - 1` 참조로 교체 — `TODO` 주석으로 마킹.
...
1. `TraceIdFilter`가 `adapter/in/web/observability/`에 위치하며 `OncePerRequestFilter`를 상속한다. MDC에 `traceId`, `method`, `path`, `query` 네 키를 주입하고 finally 블록에서 본 필터가 put한 네 키만 `MDC.remove(...)`로 제거한다 (`MDC.clear()` 금지, 코드 리뷰로 leak 방지 확인). 필터는 `@Component @Order` 대신 `FilterRegistrationBean`으로 명시 등록되며 order는 `-101` (Security 선점).
2. 모든 응답에 `X-Trace-Id` 헤더가 포함된다 (`curl -i` 출력에서 확인). 요청의 `X-Trace-Id`가 최소 위생 검증을 통과했다면 trim된 값이 응답에 echo되고, 실패했다면 새 UUID가 응답된다.
2-1. Query string 위생 처리: `?page=2&size=10&token=abc&!bad=1` 요청 시 MDC `query`는 `page=2,size=10,[1 keys redacted],[invalid]` 형태로 정확히 카디널리티 보존. allowlist는 `observability.query.value-allowlist`(default `[page, size, sort]`)에서 외부화.
...
7. **Query 키 이름 자체의 PII 누출** — value 마스킹만으로는 `?reset_token_for_alice@corp.com=...` 같이 키 이름이 PII를 드러내는 케이스를 막지 못한다. L1(키 정규식)으로 1차 차단하고, L2(default-deny `[redacted]` 익명화)로 2차 차단해 fail-safe(allowlist 갱신을 잊으면 디버깅 단서가 줄어들 뿐 누출은 없음). L3(ArchUnit + PR 템플릿 가드)는 후속 test-strategy Plan에서 강제.

8. **`FilterRegistrationBean`으로 등록 변경의 영향** — `@Component @Order` 자동 등록을 `@Bean` 명시 등록으로 바꾸면 Spring Boot가 같은 클래스를 두 번 등록할 위험이 있다. `@Component`를 제거해 single source로 만들고, 동일 패키지에 `TraceIdFilterConfig`를 두어 wiring 의도를 한 곳에 모은다.

9. **필터 내 예외는 응답 계약을 우회한다** — `TraceIdFilter`는 `DispatcherServlet` 외부의 servlet filter이므로 여기서 던진 예외는 `@RestControllerAdvice`(`GlobalExceptionHandler`)가 잡지 못하고 servlet 컨테이너의 기본 5xx로 떨어진다 → ADR-0005 §4 `ProblemDetail` 응답 계약 우회 + 외부에서 임의로 ERROR 로그를 발생시킬 수 있는 DoS 벡터(`?x=%` 등 malformed percent-encoding). **정책**: 필터의 입력 파싱(URL decode 등)은 클라이언트 입력 파싱 실패(`IllegalArgumentException` 등)를 자체 흡수해 `[invalid]`로 라우팅하고, 절대 chain 밖으로 escape시키지 않는다. 필터는 "관측 메타 추출"만 책임지며, 실패 시에도 다음 핸들러로 통과시켜 컨트롤러/`GlobalExceptionHandler`가 정상 분류하도록 한다.
...
- `docs/plans/done/PLAN-0005-B-input-validation.md` (동일 핸들러에 검증 분기 추가됨)
- `CLAUDE.md`
- 현재 상태 파악용:
...
- `src/main/java/com/dunowljj/board/adapter/in/web/observability/TraceIdFilterConfig.java` (`FilterRegistrationBean` wiring)
- `src/main/java/com/dunowljj/board/adapter/in/web/observability/QueryLogProperties.java` (`@ConfigurationProperties("observability.query")`)

#### MODIFY
- `src/main/resources/application.yml` — `observability.query.value-allowlist: [page, size, sort]` 추가

#### Tests
> Tests: 본 Plan 범위 외. 후속 test-strategy Plan에서 작성. ArchUnit으로 컨트롤러 `@RequestParam.name()` 수집 → allowlist diff 테스트도 그쪽으로.
