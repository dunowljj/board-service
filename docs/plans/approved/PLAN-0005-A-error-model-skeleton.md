...
- `GlobalExceptionHandler` 재작성: `extends ResponseEntityExceptionHandler`로 Spring MVC 프레임워크 4xx 예외(malformed JSON, 타입 불일치, missing param, method not allowed 등)의 framework-resolved status를 보존하고 body만 우리 `ErrorResponse`로 변환 (`handleExceptionInternal` override). 그 외에 `@ExceptionHandler(BusinessException.class)` 1개 + `@ExceptionHandler(Exception.class)` 안전망 1개. 기존 `PostNotFoundException` 전용 핸들러와 `IllegalArgumentException` 핸들러 제거.
