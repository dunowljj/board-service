...
- `ErrorCode` enum: `code` / `category` / `defaultMessage` 3필드. 초기 항목: `POST_NOT_FOUND` (NOT_FOUND), `INVALID_POST_CONTENT` (INVALID_INPUT), `INTERNAL_ERROR` (INTERNAL). `VALIDATION_FAILED`는 PLAN-0005-B로 미룸.
...
- `ErrorResponse` 수정: `path` 필드 추가 (`code`, `message`, `timestamp`, `path`). `errors[]`는 PLAN-0005-B에서 추가.
- `GlobalExceptionHandler` 재작성: `@ExceptionHandler(BusinessException.class)` 1개 + `@ExceptionHandler(Exception.class)` 안전망 1개. 기존 `PostNotFoundException` 전용 핸들러와 `IllegalArgumentException` 핸들러 제거.
...
6. `GlobalExceptionHandler`는 `BusinessException` 1개 + `Exception` 1개, 총 2개의 `@ExceptionHandler`만 갖는다. `PostNotFoundException` 전용 핸들러와 `IllegalArgumentException` 핸들러는 삭제됐다.
7. `ErrorResponse`는 `code`, `message`, `timestamp`, `path` 4필드를 갖는다.
...
9. 기존 외부 API 동작 회귀 없음:
   - `GET /api/posts/{id}` (없는 id) → 404, body는 `{ "code": "POST_NOT_FOUND", "message": ..., "timestamp": ..., "path": "/api/posts/{id}" }`
   - `POST /api/posts` (title 누락) → 400, body의 `code`는 `INVALID_POST_CONTENT`
...
4. **`ErrorCode.INTERNAL_ERROR`를 카탈로그에 두는 것의 오용 위험** — 도메인/유스케이스 코드가 직접 `INTERNAL_ERROR`를 throw할 수 있는 경로는 `BusinessException` 생성자의 카테고리 가드(Acceptance Criteria 4)로 차단된다. 직접 `ErrorResponse`에 `INTERNAL_ERROR`를 넣어 발행할 수 있는 곳은 `GlobalExceptionHandler` 안전망뿐임을 코드 리뷰로 확인.
...
#### MODIFY
- `src/main/java/com/dunowljj/board/adapter/in/web/dto/response/ErrorResponse.java` — `path` 필드 추가
...
- 기존 `record ErrorResponse(String code, String message, LocalDateTime timestamp)` → `record ErrorResponse(String code, String message, LocalDateTime timestamp, String path)`.
- factory 메서드: `static ErrorResponse of(ErrorCode errorCode, String path)` (defaultMessage 사용), `static ErrorResponse of(ErrorCode errorCode, String message, String path)` (override message).
- 기존 `of(String code, String message)`는 제거 (본 Plan에서 호출 지점 모두 새 시그니처로 전환).

### `adapter/in/web/exception/GlobalExceptionHandler.java`
- `@RestControllerAdvice` 유지.
- 핸들러 2개:
  1. `@ExceptionHandler(BusinessException.class)` — `ErrorCategoryHttpStatusMapper.toHttpStatus(ex.errorCode().category())`로 status 결정, `ErrorResponse.of(ex.errorCode(), request.getRequestURI())`로 body. `HttpServletRequest` 주입으로 path 획득.
  2. `@ExceptionHandler(Exception.class)` — 무조건 500 + `ErrorResponse.of(ErrorCode.INTERNAL_ERROR, request.getRequestURI())`. (현 단계에선 로깅 미포함; PLAN-0005-C에서 추가.)
