...
- 2026-06-28: 설계 근거 문서화(코드 변경 없음, ADR-0005 §5.1 amend). 리뷰 토론에서 "validator 의 null-pass(`return true`) 가 헷갈린다"는 지적 → 이것이 *footgun 이 아니라 다중 에러 동시 보고(aggregation) 를 위한 의도된 표준 관용구*임을 §5.1 에 명시. 대안 `@GroupSequence` 단락은 *전역* 단락이라 한 필드가 비면 다른 필드 형식 오류가 누락(폼 UX 왕복)되어 거부 — 가입/수정 화면엔 null-pass 유지. 함께 *복잡 검증의 애플리케이션 계층 이관 기준*(① 필드간 의존 ② I/O·외부 상태 ③ 도메인 상태 ④ 호출자/문맥 ⑤ 풍부한 에러 구조) + 리트머스("한 필드 값만으로 DB·타 필드·도메인 상태 없이 판정 가능한가 → 예: 경계 / 아니오: 애플리케이션")를 §5.1 에 추가. 향후 비번 확인 일치·조건부 필수·유출 비번 차단 등이 생기면 애너테이션 조합 대신 애플리케이션 계층 명시 검증으로 간다.
- 2026-06-28: 테스트 의무 이행(CLAUDE.md Phase 2 "tests for all changed behavior"). 초기 구현은 경계 validator 의 *동작*을 E2E(`AuthE2EIT`)와 VO 생성자 경유로만 간접 커버 — validator 클래스 자체·정책 메서드 직접 호출·byte 경계의 *빠른 단위 테스트가 비어* 있었다(archival 전 보강). 추가: (1) `ValidEmailValidatorTest`/`ValidNicknameValidatorTest` — 정상/비정상 형식 + **null/blank → true 가드** 명시 고정(정책 메서드는 blank=false 인데 validator 는 통과시키는 *의도된 비대칭*을 한 테스트에서 대비). (2) `MaxUtf8BytesValidatorTest` — 72/73 byte 경계 + 한글 1자=3byte 로 *char 아닌 byte 기준* 판정 + null-pass(reflection 으로 `@MaxUtf8Bytes(72)` 주입). (3) `EmailTest.isValid`/`NicknameTest.isValidDisplay` 정책 메서드 *직접* 호출 케이스 — "VO·validator 공유 단일 출처"를 호출자 경유가 아니라 계약 레벨에서 고정. (4) `EmailTest` 의 `case_variants_are_equal` DisplayName/본문 정정 — equals 가 ignoreCase 인 것처럼 오인되지 않게 "정규화로 동일 value→equals 동일"의 인과를 `value()` 단언으로 명시. `./gradlew test` 그린.
- 2026-06-28(리뷰 반영): 위 테스트 PR(#25) 리뷰 지적 3건 수정. **P2(버그)**: `MaxUtf8BytesValidator` 가 null 만 통과시키고 blank 는 byte 검사로 보내 `" ".repeat(73)` 같은 blank-과길이 입력에 `@NotBlank`+byte 위반 2건이 겹쳐 "중복 errors 회피" 계약 위반(ADR §5.1·원래 의도와도 불일치) → 가드를 `value == null || value.isBlank()` 로 보정해 `@ValidEmail`/`@ValidNickname` 과 동일화(애너테이션 javadoc 도 "null/blank"). 회귀 테스트 추가(`blank_passes_even_when_over_limit`). **P3**: 단위 테스트 DisplayName 과장 표현 정정 — (a) `ValidEmailValidatorTest` 의 "@ValidEmail message 가 errors[].reason 이 됨"(HTTP 매핑 주장, 단위 테스트가 검증 안 함) → "false 를 반환한다", (b) `EmailTest`/`NicknameTest` 정책 메서드 테스트의 "VO·validator 공유 단일 출처" 주장(실제론 정책 메서드만 호출) → 실제 동작 기술로 정정하고, *공유 단일 출처*의 진짜 회귀 가드는 레이어가 맞는 validator 테스트에 nonblank 동치 단언(`validator.isValid == 정책 메서드`)으로 추가.
