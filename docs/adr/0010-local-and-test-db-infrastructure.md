...
- repo root 의 `compose.yaml` 에 `pgvector/pgvector:pg16` 기반 PostgreSQL-compatible service 정의
  ...
  - container port `5432` 는 host 에 매핑한다 — Spring Boot Docker Compose 의 connection wiring 이 *mapped port 기준*. host port 는 `5432:5432` 고정 (PLAN-0010 결정 — dev 친숙성 + 현 dev DB 충돌 부재)
...
- **dev `ddl-auto: create-drop` + local 전용 seed 데이터 초기화** — dev 데이터 장기 보존 의제 부재. `application-local.yml` 이 `ddl-auto: create-drop` + PostgreSQL SQL init 활성화 + Hibernate schema 생성 이후 seed 적재 순서 + local 전용 seed 경로를 책임진다. `application.yml` 전역 default 에는 `ddl-auto` *부재* — 운영 profile 로 띄울 때 *prod schema drop 위험* 차단 (운영은 §5 의 별도 ADR 영역이지만 *안전 default* 는 본 ADR 영역). `src/main/resources/data.sql` 같은 *기본 경로* 사용 금지 — 테스트 DB 가 의도치 않게 seed 되는 위험 회피
- **seed DML 은 audit columns 를 명시한다** — JPA `AuditingEntityListener` (ADR-0008) 는 *raw SQL insert* 에 작동하지 않음. `PostJpaEntity` 의 `createdAt` / `updatedAt` 은 `nullable = false` 라 seed insert 가 *값 미명시* 시 NULL 제약 위반으로 실패. `local-data.sql` 은 `INSERT INTO posts (..., created_at, updated_at) VALUES (..., NOW(), NOW())` 형태로 작성하며, 컬럼명은 Hibernate physical naming strategy (`createdAt` → `created_at`) 를 따른다 — naming strategy 변경 시 seed 도 갱신 (PLAN-0010 결정)
...
- **`hibernate.ddl-auto` 의 profile 별 분리** — `application.yml` 전역 default 부재 (운영 profile 의 *전역 schema drop 위험* 차단). `application-local.yml` 가 `create-drop` (매 `bootRun` 마다 깨끗한 시작 + local seed 적재). test 는 PLAN-0010 의 `HibernatePropertiesCustomizer` 가 `create` override (static 공유 Testcontainer + shutdown hook 경합으로 발생하는 HHH000478 noise 회피). `application-prod.yml` 가 `validate` (entity ↔ schema 일치 검증, 불일치 시 startup 실패 — 운영 결정 전이라도 *안전 default*). 테스트 격리는 `@DataJpaTest` rollback + E2E cleanup 책임. *데이터 장기 보존 부재* 명시. migration tool (Flyway / Liquibase) 도입은 운영 시점 별도 결정
...
<!-- Testcontainers 컨테이너 라이프사이클: 해소 (PLAN-0010 결정) — static singleton (Gradle test worker JVM 당 1 개). @Bean(destroyMethod="") 로 Spring destroy 우회. reuse mode 미활성. -->
...
- **PLAN-0010** — 본 ADR 의 구현 단위. compose.yaml / build.gradle / application.yml + application-local.yml / db/seed/local-data.sql / `PostgresTestcontainersConfig` / ADR-0006 §1·§2 표 갱신. PLAN 영역 결정 4 종 (라이프사이클 = static singleton per worker JVM, host port = 5432:5432 고정, dev/test ddl-auto 분리, profile 활성화 = `spring.profiles.default`).
