...
- **내부 불변식 vs 도메인 invariant 구분.** §5 표의 "도메인 invariant → `BusinessException`(4xx)"는 *사용자 입력으로 위반될 수 있는* 불변식에 한한다. *사용자 입력 경로가 구조적으로 없는* 내부 불변식(예: `PasswordHash` 는 *해시 산출물 / DB 복원값*만 받고 평문 미접촉 — BCrypt 는 non-blank 를 반환하므로 blank 면 hasher 오설정 / DB 데이터 손상 같은 서버·데이터 조건)은 위반 시 `BusinessException`(4xx)이 아니라 *plain RuntimeException* 으로 던져 web adapter 5xx fallback(`INTERNAL_ERROR`)으로 흐른다. (DB 컬럼 `NOT NULL` 은 `null` 만 막고 `""` 는 못 막음 — 컬럼 차원 blank 차단이 필요하면 `CHECK(length(trim(password_hash)) > 0)` 는 별도 의제.) VO 가 HTTP 상태를 *판단*하지 않고 "4xx 인 척을 안 함" → 5xx 기본값(caller/상류 가드 가정 비의존).
- **`@NotBlank` 책임 분담은 다중 에러 동시 보고(aggregation)를 위한 *의도된* 선택.** 커스텀 제약(`@ValidEmail`/`@ValidNickname`/`@MaxUtf8Bytes`)의 validator 는 null/blank 를 *통과*(`return true`)시키고 존재 검사는 `@NotBlank` 가 단독으로 담당한다 — 이는 Bean Validation 스펙(JSR-380: "null elements are considered valid; presence 는 `@NotNull` 로")의 표준 관용구다. **이렇게 하는 이유는 한 요청의 모든 필드 위반을 `errors[]` 에 한 번에 모아 응답하기 위해서다.** 대안인 `@GroupSequence` 단락(존재 그룹 통과 후 형식 그룹 평가)은 필드별이 아니라 *전역* 단락이라, 한 필드가 비면 다른 필드의 형식 오류가 *그 응답에서 누락*되어 사용자를 왕복시킨다(폼 UX 퇴보). 따라서 가입/수정처럼 "모든 에러 한 번에" 화면에는 **null-pass 관용구를 유지**한다. validator 의 `return true` 는 footgun 이 아니라 이 aggregation 계약의 일부다. (`@GroupSequence` 는 *순차 검증·비싼 검사 단락*(예: 형식 통과 후에만 uniqueness 쿼리)이 이득일 때만 채택.)
- **복잡 검증의 애플리케이션 계층 이관 기준.** 경계 Bean Validation 은 *단일 필드·문맥 무관·순수(syntactic)* 검사 전용이다 — 한 필드의 raw 값만으로 통과/실패가 결정되는 경우(형식, 길이, 문자집합, 범위, 정규식). 아래 중 **하나라도** 해당하면 애너테이션 조합으로 영리하게 풀지 말고 *애플리케이션(또는 도메인) 계층에서 명시적으로 검증하고 구조화된 에러(`BusinessException` 서브타입 / 도메인 결과 타입)를 직접 생성*한다:
  1. **필드 간 의존** — 둘 이상 필드를 함께 봐야 판정(비밀번호 확인 일치, `start < end`, "type==A 면 B 필수"). 클래스-레벨 제약으로도 가능하나 가독성이 급격히 무너짐.
...
- **의도적 정보 손실 2건** — ① `@NotBlank`(null+공백 모두)와 `@NotNull`(null)이 모두 `REQUIRED` 로 수렴한다(분리는 `getInvalidValue()` 로 *가능하나* 위 확장 규약의 행동-차이 요건을 못 넘는다). 분리 시 **`MISSING` 이라는 이름은 쓰지 않는다** — Jackson 이 `{"k":null}` 과 키 부재를 모두 null 로 매핑하므로 "누락"을 주장할 수 없고, 그 구분이 진짜 필요하면 코드 어휘가 아니라 DTO 수준(`Optional`/`JsonNullable`)의 문제다. ② `@Size(max)`(char 수)와 `@MaxUtf8Bytes`(바이트)가 모두 `TOO_LONG` 이다. 클라이언트 관점에서 둘 다 "값이 너무 김"이고 차이는 `reason` 이 설명한다. 분리 필요 시 `TOO_LONG_BYTES` 추가.
