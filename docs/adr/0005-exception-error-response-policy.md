...
- **미처리 `Exception`** — *서버 내부 문제 (클라이언트가 고칠 수 없음)*
  - DB 장애, 외부 API 장애, 타임아웃, 라이브러리 NPE 등.
  - 구체 원인은 응답으로 노출하지 않는다 (보안·정보 누출 방지, 그리고 어차피 클라이언트가 손쓸 수 없음).
  - `adapter/in/web` 안전망 핸들러가 **`ErrorCode.INTERNAL_ERROR`(500)** 한 가지 응답으로 일괄 수렴시킨다.
...
RuntimeException
  ...
  │     └── ... (도메인별 추가)
  └── (그 외 모든 미처리 예외 — 5xx, 안전망이 INTERNAL_ERROR로 수렴)
```
...
- `INTERNAL_ERROR`는 카탈로그에는 있지만 어떤 `BusinessException` 서브타입과도 매핑되지 않는다. **§1의 안전망 핸들러가 직접 발행**한다.
- 이 비대칭은 §1의 분리 기준("클라이언트가 고칠 수 있는가")의 직접 결과이다: throw 측은 의도가 명확한 4xx만 다루고, 5xx는 안전망이 일괄 처리한다.
...
- 5xx 핸들러 흐름: 안전망이 `ErrorCode.INTERNAL_ERROR` 응답을 직접 발행 (§1).
...
| 안전망 `Exception` (5xx) | ERROR | 구조화 1줄 + **full stack with cause chain**. 예: `code=INTERNAL_ERROR method=POST path=/api/posts traceId=... exception=... stacktrace=...` |
...
- MDC에 `traceId`, `method`, `path`를 자동 주입한다.
...
- MDC에 `traceId`, `method`, `path` 자동 주입
- `logback-spring.xml` (또는 동등) — 운영 환경은 JSON 인코더, 개발은 텍스트 패턴
- `BusinessException` (4xx) WARN 한 줄, 안전망 `Exception` (5xx) ERROR + full stack 로깅 정책 구현 (ADR-0005 §7)
