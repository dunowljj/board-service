...
| Driven Adapter (Persistence) | JPA Entity ↔ Domain 매핑(behavior 있는 경우), Output Port 계약, auditing, dirty checking, CRUD, 페이지네이션·정렬·쿼리 | `@DataJpaTest` | slice | PostgreSQL via Testcontainers (ADR-0010) |
...
| End-to-End | 골든 플로우 1–2개 (Create → Get → Update → Delete) | `@SpringBootTest` + MockMvc | full | PostgreSQL via Testcontainers (ADR-0010) |
...
- **PostgreSQL via Testcontainers (ADR-0010 + PLAN-0010).** Persistence slice / E2E 모두 `pgvector/pgvector:pg16` Testcontainer 위에서 실행 — *prod-equiv dialect 검증*. 본래 H2 가 *임시 다리* 였고 *방언 의존 행동* 을 검증 불가했던 한계가 해소됨. *방언 의존 회피 의식 자체는 유지* — 미래 다른 dialect (e.g., 운영 DB 가 PostgreSQL 외 선택) 전환 시 재사용 가능한 방어선.
...
<!-- Testcontainers 전환 시점: 해소 (ADR-0010 + PLAN-0010). 운영 DB 결정 전에 vector 의제 진입 정합 + dialect 정합성 가치로 PostgreSQL via Testcontainers 채택. -->
- **운영 DB 선택**: ADR-0010 §5 명시 분리. 본 ADR 의 *PostgreSQL via Testcontainers* 채택이 운영 결정을 *시사* 만 함 — 별도 ADR 영역.
