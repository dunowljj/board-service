...
본 ADR 은 *코드 작성 컨벤션* 만 결정한다. *equality 가 *필요한 자리 자체* 의 결정* (예: Post Aggregate equality, PostJpaEntity equality, PostContent → record 전환) 은 *다른 ADR 영역* 으로 분리.
...
- **`PostContent` → record 전환** — record 가 *자동 equals/hashCode* 보유. 본 ADR 의 *작성 패턴* 적용 대상 자체를 제거하는 더 깔끔한 답. *value object 표현 방식* 결정으로 별도 ADR (예: ADR-0010 슬롯).
...
- **record 전환 (본 ADR 에 포함)** — 거부 사유: *value object 표현 방식* 결정 (class → record) 과 *equality 작성 패턴* 결정 (getClass → instanceof) 은 *결정 응집도 다름*. 본 ADR 이 *작성 패턴* 만 다룸. record 전환은 별도 ADR — record 가 채택되면 본 ADR 의 적용 대상 0 으로 자연 해소.
- **`AbstractValueObject` 베이스 클래스 + 추상 equals** — 거부 사유: (a) *equality 기준 필드* 가 *VO 마다 다름* — abstract 패턴이 `protected abstract List<Object> equalityFields()` 같은 *반환 보일러플레이트* 만 누적, 결국 *코드에서 동등성 기준이 *list 반환 메서드 안* 으로 숨음* → "테스트는 문서다" (ADR-0006 §4) 정신 약화. (b) Java 17 의 `record` 가 *동일 가치를 표현 자체로 해결* — abstract 베이스 패턴은 *record 도입 전 시대 잔재*. record ADR (ADR-0010 슬롯) 이 정착되면 abstract 패턴이 *더 무의미*.

## Rejected Suggestions

본 ADR 설계 과정에서 *실제로 제안되었으나 거부된* 안.

- **본 ADR 범위에 `PostContent → record` 전환 포함** — 거부 사유: 사용자 결정 (2026-05-19) — *"record를 사용하는 것은 다른 adr에 넣는 것은 어때?"*. *작성 패턴 컨벤션* 과 *value object 표현 방식* 의 응집도 분리.
...
- *PostContent record 전환* 시 (별도 ADR) 본 ADR 의 적용 대상 0 — 본 ADR 이 *짧은 수명* 일 수 있음. 다만 *작성 패턴 컨벤션* 자체는 *미래 value object 추가* 시 재활성화 가능. dormant 의식 아님.
...
- **`PostContent → record` 전환 ADR** (예: ADR-0010 슬롯) — *value object 표현 방식* 결정. record 가 *자동 equals* 라 본 ADR 의 적용 대상 0 으로 자연 해소.
...
- PLAN-0006-B Execution Notes (2026-05-08) — *PostContent record 마이그레이션 + instanceof equals 전환 deferred*. 본 ADR 이 *instanceof 부분* 회수. record 부분은 ADR-0010 슬롯.
